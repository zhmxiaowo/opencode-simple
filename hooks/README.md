...
### Require SPEC before new source files
...
```json
{
  ...
  "hooks": [{
    "type": "command",
    "command": "node -e \"const fs=require('fs');let d='';process.stdin.on('data',c=>d+=c);process.stdin.on('end',()=>{const i=JSON.parse(d);const p=i.tool_input?.file_path||'';if(/src\\/.*\\.(ts|js)$/.test(p)){const specExists=fs.existsSync('SPEC.md')||fs.readdirSync('docs').some(f=>f.startsWith('SPEC-'));if(!specExists){console.error('[Hook] No SPEC.md found — run /plan or /spec first')}}console.log(d)})\""
  }],
  "description": "Remind to create SPEC when adding new source files (Spec-First principle)"
}
```
